...
@router.get("/history")
async def get_review_history(user: User = Depends(get_current_user)):
    # Fetch approved (Sent) and Rejected (Skipped) drafts
    drafts = await Draft.find(Draft.status.in_([DraftStatus.APPROVED, DraftStatus.REJECTED])).sort("-updated_at").to_list()
