...
export function useApi() {
    ...
    return {
        ...
        getHistory: async () => {
            const res = await fetchWithAuth("/reviews/history");
            return res.json();
        },
        ...
        // Agents
        createAgent: async (agentData: Record<string, unknown>) => {
        ...
        listAgents: async (userId: string) => {
            const res = await fetchWithAuth(`/agents/?user_id=${userId}`);
            return res.json();
        },
